...
int main() {
    ...
    assert(spot_only.ok);
    assert(spot_only.decision.side == sovereign::strategies::Side::buy);

    const auto arb = sovereign::strategies::evaluateSpotFuturesArb(100.0, 101.0);
    assert(arb.ok);
    assert(arb.decision.side == sovereign::strategies::Side::sell);

    const auto options = sovereign::strategies::evaluateOptionsTrading(0.40, 0.25);
    assert(options.ok);
    assert(options.decision.side == sovereign::strategies::Side::sell);
