...
function buildStrategyPlan(name, options = {}) {
  ...
  return [
    ...
    '  universe:',
    ...universe.map((symbol) => `    - ${JSON.stringify(symbol)}`),
    '  signals:',
    ...
    '  features:',
    '    technical:',
    ...((features.technical || []).map((item) => `      - ${JSON.stringify(item)}`)),
    '    relative:',
    ...((features.relative || []).map((item) => `      - ${JSON.stringify(item)}`)),
    '    orderflow:',
    ...((features.orderflow || []).map((item) => `      - ${JSON.stringify(item)}`)),
    '    custom:',
    ...((features.custom || []).map((item) => `      - ${JSON.stringify(item)}`)),
    '  indicators:',
