...
import { buildG1Url, getG1CreditBalance, G1_CREDIT_TYPE, G1_UTM_CAMPAIGNS, isOverageEligibleModel, shouldAutoUseCredits, shouldShowEmptyWalletMenu, shouldShowOverageMenu, wrapInAccountChooser, } from './billing.js';
describe('billing', () => {
    describe('wrapInAccountChooser', () => {
        it('should wrap URL with AccountChooser redirect', () => {
            const result = wrapInAccountChooser('user@gmail.com', 'https://one.google.com/ai/activity');
            expect(result).toBe('https://accounts.google.com/AccountChooser?Email=user%40gmail.com&continue=https%3A%2F%2Fone.google.com%2Fai%2Factivity');
        });
        it('should handle special characters in email', () => {
            const result = wrapInAccountChooser('user+test@example.com', 'https://example.com');
    ...
    describe('buildG1Url', () => {
        it('should build activity URL with UTM params wrapped in AccountChooser', () => {
            const result = buildG1Url('activity', 'user@gmail.com', G1_UTM_CAMPAIGNS.MANAGE_ACTIVITY);
            ...
            expect(result).toContain('utm_campaign%3Dhydrogen_cli_settings_ai_credits_activity_page');
        });
        it('should build credits URL with UTM params wrapped in AccountChooser', () => {
            const result = buildG1Url('credits', 'test@example.com', G1_UTM_CAMPAIGNS.EMPTY_WALLET_ADD_CREDITS);
            expect(result).toContain('https://accounts.google.com/AccountChooser');
            expect(result).toContain('one.google.com%2Fai%2Fcredits');
            expect(result).toContain('utm_campaign%3Dhydrogen_cli_insufficient_credits_add_credits');
        });
    });
    ...
    describe('isOverageEligibleModel', () => {
        ...
        it('should return true for gemini-3.1-pro-preview-customtools', () => {
            expect(isOverageEligibleModel('gemini-3.1-pro-preview-customtools')).toBe(false);
        });
