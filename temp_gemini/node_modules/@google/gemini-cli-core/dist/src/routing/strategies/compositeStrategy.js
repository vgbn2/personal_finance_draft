...
export class CompositeStrategy {
    ...
    async route(context, config, baseLlmClient, localLiteRtLmClient) {
        ...
        for (const strategy of nonTerminalStrategies) {
            try {
                ...
                debugLogger.warn(`[Routing] Strategy '${strategy.name}' failed. Continuing to next strategy. Error:`, error);
            }
        }
        // If no other strategy matched, execute the terminal strategy.
        try {
            const decision = await terminalStrategy.route(context, config, baseLlmClient, localLiteRtLmClient);
            return this.finalizeDecision(decision, startTime);
        }
        catch (error) {
            coreEvents.emitFeedback('error', `[Routing] Critical Error: Terminal strategy '${terminalStrategy.name}' failed. Routing cannot proceed. Error:`, error);
            throw error;
        }
    }
    /**
