...
describe('ClearcutLogger', () => {
    ...
    describe('createLogEvent', () => {
        ...
        it('handles empty os.cpus() gracefully', async () => {
            ...
            const event = logger?.createLogEvent(EventNames.API_ERROR, []);
            const metadata = event?.event_metadata[0];
            const cpuInfoEntry = metadata?.find((m) => m.gemini_cli_key === EventMetadataKey.GEMINI_CLI_CPU_INFO);
            expect(cpuInfoEntry).toBeUndefined();
            const cpuCoresEntry = metadata?.find((m) => m.gemini_cli_key === EventMetadataKey.GEMINI_CLI_CPU_CORES);
    ...
    describe('logModelRoutingEvent', () => {
        it('logs a successful routing event', () => {
            const { logger } = setup();
            const event = new ModelRoutingEvent('gemini-pro', 'default-strategy', 123, 'some reasoning', false, undefined, ApprovalMode.DEFAULT);
        ...
        it('logs a failed routing event with a reason', () => {
            const { logger } = setup();
            const event = new ModelRoutingEvent('gemini-pro', 'router-exception', 234, 'some reasoning', true, 'Something went wrong', ApprovalMode.DEFAULT);
            logger?.logModelRoutingEvent(event);
            const events = getEvents(logger);
            expect(events.length).toBe(1);
            expect(events[0]).toHaveEventName(EventNames.MODEL_ROUTING);
        ...
        it('logs a successful routing event with numerical routing fields', () => {
            const { logger } = setup();
            const event = new ModelRoutingEvent('gemini-pro', 'NumericalClassifier (Strict)', 123, '[Score: 90 / Threshold: 80] reasoning', false, undefined, ApprovalMode.DEFAULT, true, '80');
            logger?.logModelRoutingEvent(event);
            const events = getEvents(logger);
            expect(events.length).toBe(1);
            expect(events[0]).toHaveEventName(EventNames.MODEL_ROUTING);
    ...
    describe('logHookCallEvent', () => {
        it('logs an event with proper fields', () => {
            ...
            const event = new HookCallEvent('before-tool', HookType.Command, hookName, {}, // input
            150, // duration
            true, // success
            {}, // output
            0);
