...
import { BaseSettingsDialog, } from './BaseSettingsDialog.js';
...
describe('BaseSettingsDialog', () => {
    let mockOnItemToggle;
    let mockOnEditCommit;
    let mockOnItemClear;
    let mockOnClose;
    let mockOnScopeChange;
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    const renderDialog = async (props = {}) => {
        ...
        const result = render(_jsx(KeypressProvider, { children: _jsx(BaseSettingsDialog, { ...defaultProps }) }));
    ...
    describe('rendering', () => {
        ...
        it('should render footer content when provided', async () => {
            const { lastFrame, unmount } = await renderDialog({
                footerContent: _jsx(Text, { children: "Custom Footer" }),
            });
    ...
    describe('scrolling and resizing list (search filtering)', () => {
        it('should preserve focus on the active item if it remains in the filtered list', async () => {
            ...
            const { rerender, stdin, lastFrame, waitUntilReady, unmount } = await renderDialog({
                items,
                maxItemsToShow: 5,
            });
            ...
            await act(async () => {
                rerender(_jsx(KeypressProvider, { children: _jsx(BaseSettingsDialog, { title: "Test Settings", items: filteredItems, selectedScope: SettingScope.User, maxItemsToShow: 5, onItemToggle: mockOnItemToggle, onEditCommit: mockOnEditCommit, onItemClear: mockOnItemClear, onClose: mockOnClose }) }));
            });
            await waitUntilReady();
    ...
    describe('edit mode', () => {
        it('should commit edit on Enter', async () => {
            ...
            await waitUntilReady();
            await waitFor(() => {
                expect(mockOnEditCommit).toHaveBeenCalledWith('string-setting', 'test-valuex', expect.objectContaining({ type: 'string' }));
            });
            unmount();
        });
        ...
        it('should allow number input for number fields', async () => {
            ...
            await waitUntilReady();
            await waitFor(() => {
                expect(mockOnEditCommit).toHaveBeenCalledWith('number-setting', '42123', expect.objectContaining({ type: 'number' }));
            });
            unmount();
        });
        it('should support quick number entry for number fields', async () => {
            ...
            await waitFor(() => {
                expect(mockOnEditCommit).toHaveBeenCalledWith('number-setting', '5', expect.objectContaining({ type: 'number' }));
            });
            unmount();
        });
    });
...
//# sourceMappingURL=BaseSettingsDialog.test.js.map