 ...
 */
export function colorizeCode({ code, language = null, availableHeight, maxWidth, theme = null, settings, hideLineNumbers = false, }) {
    ...
    try {
        ...
        const renderedLines = lines.map((line, index) => {
            ...
            return (_jsxs(Box, { minHeight: 1, children: [showLineNumbers && (_jsx(Box, { minWidth: padWidth + 1, flexShrink: 0, paddingRight: 1, alignItems: "flex-start", justifyContent: "flex-end", children: _jsx(Text, { color: activeTheme.colors.Gray, children: `${index + 1 + hiddenLinesCount}` }) })), _jsx(Text, { color: activeTheme.defaultColor, wrap: "wrap", children: contentToRender })] }, index));
        });
        if (availableHeight !== undefined) {
            return (_jsx(MaxSizedBox, { maxHeight: availableHeight, maxWidth: maxWidth, additionalHiddenLinesCount: hiddenLinesCount, overflowDirection: "top", children: renderedLines }));
        }
        return (_jsx(Box, { flexDirection: "column", width: maxWidth, children: renderedLines }));
    }
    catch (error) {
        ...
        const padWidth = String(lines.length).length; // Calculate padding width based on number of lines
        const fallbackLines = lines.map((line, index) => (_jsxs(Box, { minHeight: 1, children: [showLineNumbers && (_jsx(Box, { minWidth: padWidth + 1, flexShrink: 0, paddingRight: 1, alignItems: "flex-start", justifyContent: "flex-end", children: _jsx(Text, { color: activeTheme.defaultColor, children: `${index + 1}` }) })), _jsx(Text, { color: activeTheme.colors.Gray, children: line })] }, index)));
        if (availableHeight !== undefined) {
            return (_jsx(MaxSizedBox, { maxHeight: availableHeight, maxWidth: maxWidth, overflowDirection: "top", children: fallbackLines }));
        }
        return (_jsx(Box, { flexDirection: "column", width: maxWidth, children: fallbackLines }));
    }
}
