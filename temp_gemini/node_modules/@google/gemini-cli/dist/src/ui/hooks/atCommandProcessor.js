...
import { debugLogger, getErrorMessage, isNodeError, unescapePath, resolveToRealPath, fileExists, ReadManyFilesTool, REFERENCE_CONTENT_START, REFERENCE_CONTENT_END, CoreToolCallStatus, } from '@google/gemini-cli-core';
...
function categorizeAtCommands(commandParts, config) {
    const agentParts = [];
    const resourceParts = [];
    const fileParts = [];
 ...
 */
export async function checkPermissions(query, config) {
 ...
 */
async function resolveFilePaths(fileParts, config, onDebugMessage, signal) {
                        ...
                        try {
                            ...
                            if (globResult.llmContent &&
                                ...
                                if (lines.length > 1 && lines[1]) {
                                    ...
                                    onDebugMessage(`Glob search for ${pathName} found ${firstMatchAbsolute}, using relative path: ${pathSpec}`);
                                    break;
                                }
                                else {
 ...
 */
function constructInitialQuery(commandParts, resolvedFiles) {
 ...
 */
async function readMcpResources(resourceParts, config, signal) {
    const resourceRegistry = config.getResourceRegistry();
 ...
 */
async function readLocalFiles(resolvedFiles, config, signal, userMessageTimestamp) {
            ...
            for (const part of result.llmContent) {
                if (typeof part === 'string') {
                    ...
                    if (match) {
                        ...
                        const resolvedFile = resolvedFiles.find((rf) => rf.absolutePath === filePathSpecInContent ||
 ...
 */
function reportIgnoredFiles(ignoredFiles, onDebugMessage) {
