...
import { FileDiscoveryService, GlobTool, ReadManyFilesTool, StandardFileSystemService, ToolRegistry, COMMON_IGNORE_PATTERNS, GEMINI_IGNORE_FILE_NAME, 
...
describe('handleAtCommand', () => {
    ...
    beforeEach(async () => {
        ...
        mockConfig = {
            ...
            getWorkspaceContext: () => ({
                isPathWithinWorkspace: (p) => p.startsWith(testRootDir) || p.startsWith('/private' + testRootDir),
            ...
            isPathAllowed(absolutePath) {
                ...
                return (absolutePath.startsWith(resolvedProjectTempDir + path.sep) ||
    ...
    it('should correctly handle double-quoted paths with spaces', async () => {
        ...
        const filePath = await createTestFile(path.join(testRootDir, 'my folder', 'my file.txt'), fileContent);
    ...
    it('should correctly handle file paths with narrow non-breaking space (NNBSP)', async () => {
        ...
        const filePath = await createTestFile(path.join(testRootDir, `my${nnbsp}file.txt`), fileContent);
        const relativePath = getRelativePath(filePath);
        const query = `@${filePath}`;
        const result = await handleAtCommand({
            query,
    ...
    describe('gemini-ignore filtering', () => {
        it('should skip gemini-ignored files in @ commands', async () => {
            await createTestFile(path.join(testRootDir, GEMINI_IGNORE_FILE_NAME), 'build/output.js');
    ...
    it('should handle mixed gemini-ignored and valid files', async () => {
        await createTestFile(path.join(testRootDir, GEMINI_IGNORE_FILE_NAME), 'dist/bundle.js');
    ...
    it('should include agent nudge when agents are found', async () => {
        ...
        mockConfig.getAgentRegistry = vi.fn().mockReturnValue({
            getDefinition: (name) => name === agentName || name === otherAgent ? { name } : undefined,
        });
