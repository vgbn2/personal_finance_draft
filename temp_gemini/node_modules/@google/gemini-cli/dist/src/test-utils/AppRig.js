...
import { makeFakeConfig, ExtensionLoader, AuthType, ApprovalMode, createPolicyEngineConfig, PolicyDecision, ToolConfirmationOutcome, MessageBusType, coreEvents, ideContextStore, createContentGenerator, IdeClient, debugLogger, CoreToolCallStatus, } from '@google/gemini-cli-core';
...
vi.mock('../ui/components/GeminiRespondingSpinner.js', async () => {
    ...
    return {
        ...
        GeminiRespondingSpinner: ({ nonRespondingDisplay, }) => React.createElement(Text, null, nonRespondingDisplay || '...'),
    };
});
export class AppRig {
    options;
    ...
    constructor(options = {}) {
        ...
        this.testDir = fs.mkdtempSync(path.join(os.tmpdir(), `gemini-app-rig-${uniqueId.slice(0, 8)}-`));
    ...
    isBusy() {
        ...
        const isAwaitingConfirmation = this.toolCalls.some((tc) => tc.status === CoreToolCallStatus.AwaitingApproval);
    ...
    removeToolPolicy(toolName, source = 'AppRig Override') {
        ...
        this.config
            ...
            .removeRulesForTool(actualToolName, source);
     ...
     */
    async waitForNextEvent(timeout = 60000) {
        let confirmation;
        let isIdle = false;
     ...
     */
    async drainBreakpointsUntilIdle(onConfirmation, timeout = 60000) {
        while (true) {
