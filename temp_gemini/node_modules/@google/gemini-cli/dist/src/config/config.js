...
import { setGeminiMdFilename as setServerGeminiMdFilename, getCurrentGeminiMdFilename, ApprovalMode, DEFAULT_GEMINI_EMBEDDING_MODEL, DEFAULT_FILE_FILTERING_OPTIONS, DEFAULT_MEMORY_FILE_FILTERING_OPTIONS, FileDiscoveryService, WRITE_FILE_TOOL_NAME, SHELL_TOOL_NAMES, SHELL_TOOL_NAME, resolveTelemetrySettings, FatalConfigError, getPty, EDIT_TOOL_NAME, debugLogger, loadServerHierarchicalMemory, WEB_FETCH_TOOL_NAME, ASK_USER_TOOL_NAME, getVersion, PREVIEW_GEMINI_MODEL_AUTO, coreEvents, GEMINI_MODEL_ALIAS_AUTO, getAdminErrorMessage, isHeadlessMode, Config, applyAdminAllowlist, getAdminBlockedMcpServersMessage, } from '@google/gemini-cli-core';
...
export async function parseArguments(settings) {
    ...
    if (q && !result['prompt']) {
        if (!isHeadlessMode()) {
            startupMessages.push('Positional arguments now default to interactive mode. To run in non-interactive mode, use the --prompt (-p) flag.');
...
export async function loadCliConfig(settings, sessionId, argv, options = {}) {
    ...
    const folderTrust = process.env['GEMINI_CLI_INTEGRATION_TEST'] === 'true' ||
        process.env['VITEST'] === 'true'
        ? false
        : (settings.security?.folderTrust?.enabled ?? false);
    const trustedFolder = isWorkspaceTrusted(settings, cwd, undefined, {
        prompt: argv.prompt,
        query: argv.query,
    })?.isTrusted ?? false;
    ...
    if (!experimentalJitContext) {
        ...
        const result = await loadServerHierarchicalMemory(cwd, settings.context?.loadMemoryFromIncludeDirectories || false
            ? includeDirectories
    ...
    let approvalMode;
    const rawApprovalMode = argv.approvalMode ||
        ...
        switch (rawApprovalMode) {
            ...
            case 'plan':
                if (!(settings.experimental?.plan ?? false)) {
                    debugLogger.warn('Approval mode "plan" is only available when experimental.plan is enabled. Falling back to "default".');
    ...
    const policyEngineConfig = await createPolicyEngineConfig(effectiveSettings, approvalMode, workspacePoliciesDir);
    policyEngineConfig.nonInteractive = !interactive;
    const defaultModel = PREVIEW_GEMINI_MODEL_AUTO;
    const specifiedModel = argv.model || process.env['GEMINI_MODEL'] || settings.model?.name;
    const resolvedModel = specifiedModel === GEMINI_MODEL_ALIAS_AUTO
        ? defaultModel
