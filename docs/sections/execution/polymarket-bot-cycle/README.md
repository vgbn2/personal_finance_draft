...
The architecture-level paper-ledger boundary is described in [System Architecture & Codebase](../../../engineering/architecture/01_ARCHITECTURE_AND_CODEBASE.md). That ledger is authoritative for internal paper events; this older bot cycle still maintains a separate `bot_state.json` projection and has not converged to the ledger.
