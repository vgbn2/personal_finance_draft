        ...
        try:
            ...
            for detail in deets:
                        ...
                        if row['3_hour'] > params['volatility_threshold'] or price_change >= 0.05:
                            ...
                            if overall_ratio < 0:
                                ...
                                # 3. Our current order is too large and needs to be resized
                                    ...
                                    send_buy_order(order)
                                # Logic for cancelling orders when market conditions change
                                elif second_best_bid is not None and best_bid_size < orders['buy']['size'] * 0.98 and abs(best_bid - second_best_bid) > 0.03:
                ...
                # ------- TAKE PROFIT / SELL ORDER MANAGEMENT -------            
                    ...
                    # Additional conditions for updating sell orders
                    elif orders['sell']['price'] < ask_price:
                        print(f"Updating Sell Order for {token} because its not at the right price")
                        send_sell_order(order)
                    elif second_best_ask is not None and best_ask_size < orders['sell']['size'] * 0.98 and abs(best_ask - second_best_ask) > 0.03:
