...
| `backend/scripts/data_ops/ingest_market_data` | working tree | 2026-07-05 | B- | connective sweep + explicit-stub fix (s64) | Enabled placeholder lanes now fail with structured `not_implemented` errors instead of returning `{}`. Real provider implementation is still pending for `pmi`, `flight`, `crypto_tx`, `holdings`, `onchain`, and `breadth`. |
