...
| `backend/gateway` | `111b1f6f` | 2026-07-24 | C+ / ledger-gated | paper state, runtime policy, aggregate test, and native-risk sweep (s99) | Focused bot/risk tests pass; paper event atomicity and one effective runtime policy remain open. |
...
| `shared/lib/market` | `111b1f6f` + local data | 2026-07-24 | B- / freshness-gated | read-only integrity and freshness sweep (s99) | 92/92 cached, 87 required-window stale, 9 cadence-plausible, 0 unexplained grain; target writer host remains unproven. |
...
| `tests` | `111b1f6f` + working tree | 2026-07-24 | C / isolation-gated | archive, hygiene, aggregate-vs-focused, and test-state sweep (s99) | Focused files pass, but aggregate Node is 122/138 with 16 order/state-sensitive failures in both default and serial runs. |
| `docs` testing/deployment guidance | `111b1f6f` + working tree | 2026-07-24 | C+ / architecture-parity-gated | architecture view, command wiring, and docs claim sweep (s99) | Operational guides mostly match; `architecture_overview.md` still says broker routing is planned and trading modules do not compile. |
