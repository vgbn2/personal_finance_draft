...
|---|---|---|---|---|---|
| system design (cross-cutting) | `87d896de` + working tree | 2026-07-24 | C+ / operations-and-release-gated | runtime policy, paper lifecycle, remote client, freshness, and host-proof sweep (s101) | The observed paper lifecycle defects and remote-client repository gap are closed; persistent-host freshness, login/reconnect, recovery/soak, and committed-source proof remain absent. |
...
| `backend/api` | `87d896de` + working tree | 2026-07-24 | A- / host-runtime-gated | scoped client-token, cached endpoint, auth-boundary, and API aggregate sweep (s101) | Read-only client status/bias routes accept a distinct scoped token without granting writes or admin paths; API 10/10 passes. Target-host runtime remains external. |
...
| `backend/gateway` | `87d896de` + working tree | 2026-07-24 | B / convergence-gated | bot dry-run exit and repeated-token settlement lifecycle implementation (s101) | Unpriced positions remain open and reopened-token settlements use lifecycle identity; canonical bot-state projection remains a broader G3 gate. |
| `backend/cli` | `87d896de` + working tree | 2026-07-24 | A- / host-runtime-gated | remote cached views, token configuration, watch/reconnect, and no-poll sweep (s101) | Remote CLI reads scoped cached endpoints and exposes explicit stale/unavailable states; real tunnel/login behavior remains external. |
...
| `infra/client` | `87d896de` + working tree | 2026-07-24 | B+ / real-login-gated | Linux user-systemd, Windows scheduled-task, secret handling, reconnect, and uninstall sweep (s101) | Contract and parser tests pass with auto-open disabled by default; no actual service/task/tunnel was installed or exercised. |
| `tests` | `87d896de` + working tree | 2026-07-24 | A- / external-runtime-gated | paper lifecycle/concurrency, API auth, remote CLI, startup contracts, and aggregate sweep (s101) | Observed paper regressions and independent-review hardening are covered; final host aggregate is 894/890/0fail/4skip. Real host/login/reconnect/soak remains external. |
| `docs` testing/deployment guidance | `87d896de` + working tree | 2026-07-24 | A- / host-proof-gated | remote client, private tunnel, distinct-token, runtime policy, and blocker sweep (s101) | Operator guidance now separates client connector from central polling/writing and defaults CLI auto-open off; real host recovery/soak evidence remains external. |
| `workspace` continuity | `87d896de` + working tree | 2026-07-24 | A- / current | implementation, review, grades, prompt, DCS, and proof-boundary sync (s101) | Independent review findings are closed; the dirty source boundary remains explicit. Next is commit/clean-source proof, then approved host freshness and startup validation. |
