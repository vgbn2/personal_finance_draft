...
| `infra/docker` + `infra/systemd` + `backend/scripts/ops` | `df3c5c57` | 2026-07-22 | A- / real-host-gated | local env isolation, NVM/service-group, Compose/config sweep (s90) | Local validation is green and this laptop is explicitly testing-only; no timer, poller, or persistent host exists. |
...
| `tests` | `df3c5c57` | 2026-07-22 | A- / broad-gate | central-env plus deployment/config matrix (s90) | Node 846/842/0fail/4skip and focused deployment 4/4; no local daemon/runtime test was authorized. |
| `docs` testing/deployment guidance | `df3c5c57` | 2026-07-22 | A- / host-unselected | testing-only correction, env preparation, GitHub-vs-host, private-bind truth sweep (s90) | Local config validation is separated from persistent hosting; this laptop is not presented as the central writer. |
| `workspace` continuity | session 90 closeout | 2026-07-22 | A- / current | testing-only correction, no-host status, verification and DCS gate | Paid hosting and this laptop are both excluded; the next runtime gate waits for a separate zero-cost persistent machine. |
