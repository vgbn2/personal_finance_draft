...
6. Validate working directory (`PWD`): ensure working directory is `REPO_ROOT` to prevent nested path slippage (e.g. `workspace/home/`).
7. Refresh `graphify-out` only when code changed and the graph tool exists. Otherwise record the unavailable gate once and use `rg` plus direct reads.
8. Record the prompt in `workspace/PROMPT_LOG.md` when writes are allowed. In Plan Mode or another read-only mode, defer the entry until an authorized closeout.
...
2. Run `node scripts/dev/check_hygiene.js` and `npm run test:structure` to ensure zero path slippage, untracked scratch artifacts, or broken links remain.
3. Append verified results and open work to today's dated handoff under `workspace/handoff/YYYY/MM/YYYY-MM-DD.md` (e.g., `workspace/handoff/2026/08/2026-08-15.md`).
4. Append durable facts and cautions to `workspace/SESSION_MEMORY.md`.
5. Update `workspace/STATE.md` only when project direction, behavior, or a grade-relevant fact changed.
6. Update `workspace/NEXT_SESSION_GOAL.md` when the next action changed.
7. Refresh graph artifacts after meaningful code changes when tooling exists.
8. Report commands run, pass/fail/blocked evidence, untouched safety boundaries, and the first next action.
