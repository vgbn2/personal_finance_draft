...
### Dual-Wielding Protocol (N-Agent Mode)

**Permission**: Full authorization granted to utilize `npx @google/gemini-cli` as a parallel processing agent.

**Usage Rules:**
1. **Offloading**: Use the CLI sub-agent for terminal-heavy tasks (research, builds, verification) to preserve Antigravity token budget and reduce rate limits.
2. **State Sync**: Sub-agents MUST report findings back to `STATE.md` or a dedicated log before major task boundaries.
3. **Collaboration**: Antigravity acts as "Lead Orchestrator"; Gemini CLI acts as "Execution/Scout Agent".
