...
test('gateway paper paths do not initialize credentialed execution adapters', () => {
  ...
  assert.match(gateway, /const adapter =\s*(?:broker === 'mt5'\s*\?\s*new Mt5Adapter\(\)\s*:\s*)?(?:\(broker === 'gate_io' \|\| broker === 'gateio'\)\s*\?\s*new GateIoAdapter\([^)]*\)\s*:\s*)?isLive\s*\?\s*new AlpacaAdapter/);
