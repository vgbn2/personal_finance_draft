...
test('verification atomically replaces stale pass evidence with a failed manifest', (t) => {
  ...
  const failed = runVerification({
    ...
    commandRunner() {
      ...
      if (calls === 1) {
        ...
        assert.equal(inProgress.failure_reason, 'verification_in_progress');
      }
      return calls === 1
        ? { status: 7, signal: null, stdout: '', stderr: 'fixture failure\n' }
        : fakePass();
    },
    output: silentOutput(),
  });
  ...
  assert.doesNotMatch(JSON.stringify(retained), /fixture failure/);
  assert.deepEqual(
